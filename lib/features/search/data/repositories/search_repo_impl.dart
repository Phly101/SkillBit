...
class SearchRepoImpl implements SearchRepository {
  ...
  @override
  Future<Either<Failure, List<HomeCourseModel>>> searchCourses(
    final String courseQuery,
  ) async {
    try {
      final List<HomeCourseModel> result = await searchLocalDataSource
          .searchCourses(courseQuery);
      return Right<Failure, List<HomeCourseModel>>(result);
    } catch (e) {
      return Left<Failure, List<HomeCourseModel>>(CacheFailures());
    }
  }
